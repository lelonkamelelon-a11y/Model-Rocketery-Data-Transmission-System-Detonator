...
void loop() {
  if (radio.available()) {
    ...
    // --- NOWY KOD: KOMENDA ABORT (Wyłączenie przekaźnika) ---
    if (strstr(text, "ABORT")) {
      Serial.println("--> PRZERWANO - PRZEKAŹNIK OFF (Stan HIGH)");
      
      digitalWrite(relayPin, HIGH); // Wyłącza przekaźnik
      
      // Krótki sygnał buzzera informujący o odcięciu
      digitalWrite(buzzerPin, HIGH);
      delay(100);
      digitalWrite(buzzerPin, LOW);
    }
